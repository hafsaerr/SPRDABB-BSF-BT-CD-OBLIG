...
def _page_spread() -> None:
    ...
    df_display = df_work[disp].reset_index(drop=True).copy()
    for _col in ["Taux BDT", "Taux instrument", "Spread (bps)", "Maturité (ans)"]:
    ...
    if not df_tcn_bt.empty:
        ...
        # Si le filtre 10-70 bps ne retient rien, exporter tout sans filtre
        df_export = df_xls_filt if not df_xls_filt.empty else df_xls

        output_tcn = io.BytesIO()
        with pd.ExcelWriter(output_tcn, engine="openpyxl") as writer:
            _write_sheet_tcn(writer, df_export, "TOUT")
            df_export = df_export.copy()
            df_export["_type_nn"] = df_export["Type"].fillna("AUTRE")
            df_export["_bank_nn"] = df_export["_bank"].fillna("AUTRE")
            for (typ, bank), df_grp in df_export.groupby(["_type_nn", "_bank_nn"]):
                if not df_grp.empty:
