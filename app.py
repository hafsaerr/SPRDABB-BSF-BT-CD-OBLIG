...
def _page_spread() -> None:
    ...
    if missing_dates:
        ...
        # Pour chaque date manquante, utiliser la courbe du jour le plus proche disponible
        for d in missing_dates:
            for delta in range(1, 6):
    ...
    if not df_tcn_bt.empty:
        ...
        if has_spread:
            ...
            df_xls_filt = df_xls[spread_num.between(10, 70)].copy()
        else:
        ...
        if df_xls_filt.empty:
            st.warning("Aucun instrument CD/BSF/BT avec spread entre 10 et 70 bps.")
            return
