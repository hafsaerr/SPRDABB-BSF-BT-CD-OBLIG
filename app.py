...
def _page_spread() -> None:
    ...
    if not df_tcn_bt.empty:
        ...
        # Export tous les instruments sans filtre de spread
        ...
        n_total = len(df_xls)
        if has_spread:
            n_with_spread = int(pd.to_numeric(df_xls["Spread (bps)"], errors="coerce").between(10, 70).sum())
            st.info(f"Export CD/BSF/BT : **{n_total}** instruments exportés "
                    f"({n_with_spread} avec spread entre 10 et 70 bps).")
        ...
        _exp_cols = [c for c in _exp_cols if c in df_xls.columns]
